...
from trains.singleTask.model import DHFN
...
def DHFN_run(
    ...
    seeds = seeds if seeds != [] else [1112, 1113, 1114, 1115,1111]
    logger = _set_logger(log_dir, model_name, dataset_name, verbose_level)
    

    args = get_config_regression(model_name, dataset_name, config_file)
    # ===== DEBUG: 验证配置 =====
    print(f"🔍 need_data_aligned: {args.need_data_aligned}")
    if hasattr(args, 'len_a'):
        print(f"🔍 模型音频长度: {args.len_a}")
    # ===========================
...
def _run(args, num_workers=4, is_tune=False, from_sena=False): 
    ...
    if args.is_training:
        ...
        model_DHFN = model_DHFN.cuda()

        model = [model_DHFN]
    else:
        print("testing phase for DHFN")
        model = DHFN(args)
        model = model.cuda()
    ...
    #test
    if args.mode == 'test':
        model.load_state_dict(torch.load('./pt/DHFN'+str(args.dataset_name)+'.pth'),strict=False)
    ...
    #train
        ...
        model[0].load_state_dict(torch.load('./pt/DHFN'+str(args.dataset_name)+'.pth'))
